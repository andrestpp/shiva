...
let cli_send_transfast_email (mt_order: Money_transfer__Store.order) (mt_sender_doc: Money_transfer__Store.sender_document) =
  Printf.sprintf
    "./cli queue -t transfast.documents -d '{\"documents\": [{\"senderId\": %d, \"type\": 31, \"subtype\": \"pdf\", \"url\": \"%s\"}], \"reference\": %d}'" mt_order.sender_id mt_sender_doc.url mt_order.id
...
let send_transfast_email order_code _verbose =
  ...
  try
    let senda_order = Senda.get_order senda_conn order_code in
    let mt_order = Money_transfer.get_order mt_conn senda_order.id in
    let mt_sender_doc = Money_transfer.get_sender_document mt_conn mt_order.sender_id in
